...
@section('content')
    <div x-data="categoryForm()" x-init="init()">
        ...
        <div class="bg-white border-b border-gray-100 px-5 py-4 flex items-center gap-4">
            ...
            <h1 class="text-lg font-semibold text-gray-800">Add Category</h1>
        </div>
        ...
        <form method="POST" action="{{ route('categories.store') }}">
            ...
            <div class="px-5 pt-4">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach (['#ef4444', '#f97316', '#eab308', '#22c55e', '#14b8a6', '#06b6d4', '#3b82f6', '#6366f1', '#8b5cf6', '#ec4899', '#f43f5e', '#78716c'] as $color)
                        <button type="button" @click="color = '{{ $color }}'"
                            ...
                            :class="color === '{{ $color }}' ? 'border-gray-800 scale-110' :
                ...
                <div class="flex items-center gap-3 mt-3">
                    <input type="color" x-model="color" aria-label="Custom category color"
                        class="w-12 h-10 rounded-lg border-0 p-1 cursor-pointer">
                    <input type="text" name="color" x-model="color" maxlength="20" placeholder="#3b82f6"
            ...
            <div class="px-5 pt-4 pb-4">
                ...
                <div class="grid grid-cols-8 gap-2">
                    @foreach (['🍔', '🛒', '🚗', '🏠', '🎮', '📚', '✈️', '☕', '🎬', '💰', '🎁', '📱', '💻', '🎓', '💊', '🏥', '🚌', '🎵', '🐾', '👶', '💼', '🔧', '💡', '💳', '🏦', '📊', '🎯', '🔖'] as $icon)
                        <button type="button" @click="icon = '{{ $icon }}'"
                            ...
                            :class="icon === '{{ $icon }}' ? 'bg-blue-100 ring-2 ring-blue-500' :
                ...
                <div class="flex items-center gap-3 mt-4">
                    <div class="w-11 h-11 rounded-xl flex items-center justify-center text-xl text-white shrink-0"
                        :style="`background-color: ${color}`" x-text="icon || '🔖'"></div>
                    <input type="text" name="icon" x-model="icon" maxlength="20" placeholder="Custom icon or emoji"
                        class="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
